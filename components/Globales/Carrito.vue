<template>
  <Popover class="relative">
    <PopoverButton class="p-2 rounded-full hover:bg-gray-300/20">
      <div class="relative">
        <div class="absolute z-50 bg-green-500 dark:bg-red-500 h-1 w-1 rounded-full animate-pulse" v-if="carrito.hasItems"></div>
      </div>
    ...
    <transition enter-active-class="transition duration-200 ease-out" enter-from-class="translate-x-10 opacity-0"
      ...
      <PopoverPanel
        class="absolute z-10 bg-slate-50 border rounded left-0 mt-4 w-screen max-w-[290px] -translate-x-[68%] transform p-4 lg:p-5 sm:px-0 lg:max-w-sm dark:bg-gray-900 dark:border-gray-800">
          ...
          <div v-else>
            <div v-for="(producto, index) in items" :key="producto.id" class="flex flex-wrap border dark:border-b dark:border-b-gray-700 dark:border-t-0 dark:border-l-0 dark:border-r-0">
              <div class="flex h-12 w-auto">
                <img class="h-full lg:rounded" :src="fileUrl(producto.Imagen_Principal)" alt="">
              </div>
              <div class="ml-3 lg:mb-3">
                <h3 class="font-primary text-lg tracking-wide text-gray-400">{{ producto.Nombre }}</h3>
                <p class="font-mono text-gray-700 dark:text-gray-400">{{ formatCurrency(producto.Propiedades.Precio * producto.quantity) }} | <span class="text-sm">{{ formato !== 'EU' ? mapaTallas[formato][producto.Genero][producto.Propiedades.Talla] : producto.Propiedades.Talla}}</span><span class="text-xs"> x{{ producto.quantity}}</span></p>
              </div>
            ...
            <div class="flex items-center mt-3">
              <VButton variant="default" class="font-primary">GO TO CHECKOUT</VButton>
