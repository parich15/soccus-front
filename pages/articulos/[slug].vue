<template>
    <div v-if="!pending" class="max-w-3xl px-6 py-24 mx-auto space-y-8">
      ...
      <div class="flex">
        <VButton v-if="producto.Propiedades[indice].Stock >= 1" variant="secondary" :class="added ? 'bg-gray-400 text-gray-200 dark:bg-gray-700':'bg-gray-200'" class=" outline-none ring-0 w-full justify-center"
      ...
      <div>
        ...
        <div class="flex flex-row mx-2 gap-x-3">
          <div :class="indice == index ? 'border-2 border-gray-500 dark:border-gray-300':' '" class="border px-4 py-3 dark:border-gray-700" @click="indice = index" v-for="(propiedad,index) in producto.Propiedades" :key="propiedad.id">
