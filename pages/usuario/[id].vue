<template>
    <div v-if="!pending">   
       <main class="pt-24 pb-10">
           <section class="max-w-sm mx-auto container">
                <div class="my-5 ml-3">
                    <span class="text-sm font-primary text-gray-300">WELCOME BACK</span>
                    <h2 class="font-primary text-5xl text-gray-800 mt-1 dark:text-gray-600">{{ user.first_name }}</h2>
                </div>
                <div class="bg-gray-50 dark:bg-gray-950 rounded p-3 mt-10">
                    <div class="flex justify-between">
                        <h5 class="font-mono text-gray-400 text-sm">Datos de cliente</h5>
                        <button @click="editMode = true" v-if="!editMode && user.Telefono && user.Direccion" class="font-mono text-gray-400 text-sm">Editar</button>
                    </div>
                    <div class="my-4">
                        <div v-if="!editMode && user.Telefono == null && user.Direccion == null" class="border border-dashed p-1  flex items-center justify-center group hover:border-solid">
                            <button @click="editMode = true" class="p-3 w-full font-primary text-xl text-gray-400 group-hover:text-gray-300 transition-all group-hover:scale-95">+ Añadir datos</button>
                        </div>
                        ...
                        <form v-if="editMode" class="flex flex-col gap-2" @submit.prevent="updateData">
                            ...
                            <VInput id="Telefono" type="number" inputmode="number" required v-model="telefono" class="font-mono text-gray-800"></VInput>

                            <label for="CP" class="font-mono text-sm text-gray-500">Codigo Postal</label>
                            <VInput id="CP" type="number" inputmode="number" required v-model="cp" class="font-mono text-gray-800"></VInput>

                            <label for="Pais" class="font-mono text-sm text-gray-500">Pais</label>
                            <VInput id="Pais" type="text" inputmode="number" required v-model="pais" class="font-mono text-gray-800"></VInput>
                            ...
                            <div class="flex justify-around mt-2">
                                <VButton type="submit" variant="secondary" class="bg-red-500 dark:bg-green-400  text-white font-primary tracking-wider disabled:bg-red-300 dark:disabled:bg-green-200" :disabled="!formCompleto">Guardar</VButton>
                ...
                <div class="mt-5 bg-gray-50 dark:bg-gray-950 rounded p-3" id="pedidos">
                    <div class="flex">
                        <h5 class="font-mono text-gray-400 text-sm">Pedidos</h5>
                    </div>
                    <div v-if="!pedidos || pedidos.length == 0">
                        <h3 class="my-2 font-primary text-xl text-gray-600">Sin Pedidos</h3>
                    </div>
                    <div v-else>
                        <div v-for="pedido in pedidos" :key="pedido.id">
                            <div v-for="producto in pedido.Productos" :key="producto" class="flex flex-row mx-1 items-center" @click="$router.push(`/articulos/${producto.Productos_id.slug}`)">
                                <img class="h-12 w-12 rounded" :src="fileUrl(producto.Productos_id.Imagen_Principal)" alt="">
                                <div class="flex flex-col">
                                    <h4 class="font-primary text-xl ml-3">{{ producto.Productos_id.Nombre }}</h4>
